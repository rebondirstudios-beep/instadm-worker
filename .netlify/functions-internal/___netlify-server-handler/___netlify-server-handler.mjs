...
} from '/var/task/instadm-clone/.netlify/dist/run/handlers/request-context.cjs'
import { getTracer, withActiveSpan } from '/var/task/instadm-clone/.netlify/dist/run/handlers/tracer.cjs'

process.chdir('/var/task/instadm-clone')
  ...
  const handlerResponse = await runWithRequestContext(requestContext, () => {
    return withActiveSpan(tracer, 'Next.js Server Handler', async (span) => {
      span?.setAttributes({
        ...
        monorepo: true,
        cwd: '/var/task/instadm-clone',
      })
      if (!cachedHandler) {
        const { default: handler } = await import('/var/task/instadm-clone/.netlify/dist/run/handlers/server.js')
